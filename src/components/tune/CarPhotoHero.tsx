...
export function CarPhotoHero({
  ...
  const aspect = compact ? "aspect-[3/1]" : "aspect-[16/9]";
      ...
      <div className={`relative ${aspect} w-full overflow-hidden`}>
        ...
        <div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-3 px-5 pb-5">
          <div>
            <div
              className={[
                "font-[family-name:var(--ts-font-heading)] font-[number:var(--ts-heading-weight)] text-white",
          ...
          {!compact && (
            <span className="rounded-full bg-[var(--ts-accent-soft)] px-3 py-1 font-[family-name:var(--ts-font-mono)] text-xs font-medium text-[var(--ts-accent)]">
              {driveType}
            </span>
          )}
        </div>
