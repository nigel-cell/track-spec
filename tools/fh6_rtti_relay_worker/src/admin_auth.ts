...
export async function authenticateAdmin(request: Request, rawBody: string, secret: string): Promise<AdminAuthResult> {
  ...
  if (!/^[A-Za-z0-9._:-]{16,128}$/.test(requestId)) {
    return { ok: false, requestId, error: "invalid admin request id" };
  }
